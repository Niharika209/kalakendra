...
function LoginPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    // Simple client-side "login" — store user in localStorage
    ...
    <>
      ...
      <main className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="w-full max-w-md p-8 bg-white border border-amber-100 rounded-lg shadow-md">
          <h1 className="text-2xl font-bold text-amber-900 mb-4">Login</h1>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-amber-900 mb-1">Name (optional)</label>
          ...
          <p className="text-sm text-amber-700 mt-4">Don't have an account? <Link to="/auth/signup" className="text-amber-900 font-semibold">Sign up</Link></p>
        </div>
      </main>
    </>
  )
